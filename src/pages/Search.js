...
export const Search = () => {
  ...
  useEffect(() => {
    let here = true;
    try {
      let url = `https://api.themoviedb.org/3/search/movie?api_key=89c2f4cad3722f5e0fd78a89c8d7a6e8&language=en-US&query=${query}&limit=${limit}&page=${nextPage}`;
  ...
  }, [changeUrl, limit, nextPage, query]);
  const handleFilter = () => {
    setFilter(true);
  };
  useEffect(() => {
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mt-5 mb-10 flex flex-col gap-5 md:items-center md:flex-row">
        <select
        ...
        <select
          ...
          <option value="DEFAULT" selected className="text-black">
            select Year
          </option>
